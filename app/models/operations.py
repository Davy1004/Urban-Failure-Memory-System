...
class Intervention(Base):
    ...
    location_id: Mapped[int] = mapped_column(UInt, ForeignKey("locations.location_id", ondelete="CASCADE", name="fk_intervention_location"), nullable=False)
    ...
    external_ref: Mapped[Optional[str]] = mapped_column(String(120), comment="work order number")
    source_id: Mapped[Optional[int]] = mapped_column(USmallInt, ForeignKey("data_sources.source_id", ondelete="SET NULL", name="fk_intervention_source"))
...
class InterventionEffect(Base):
    ...
    intervention_id: Mapped[int] = mapped_column(UInt, ForeignKey("interventions.intervention_id", ondelete="CASCADE", name="fk_effect_intervention"), nullable=False)
    ...
    did_estimate: Mapped[Optional[Decimal]] = mapped_column(Numeric(9, 5), comment="negative = failures reduced")
    p_value: Mapped[Optional[Decimal]] = mapped_column(Numeric(8, 7))
    verdict: Mapped[EffectVerdict] = mapped_column(Enum(EffectVerdict, values_callable=lambda e: [m.value for m in e]), nullable=False, default=EffectVerdict.INCONCLUSIVE, server_default="inconclusive")
...
class Alert(Base):
    ...
    prediction_id: Mapped[Optional[int]] = mapped_column(UBigInt, ForeignKey("risk_predictions.prediction_id", ondelete="CASCADE", name="fk_alert_prediction"))
    emerging_id: Mapped[Optional[int]] = mapped_column(UInt, ForeignKey("emerging_locations.emerging_id", ondelete="CASCADE", name="fk_alert_emerging"))
    city_id: Mapped[int] = mapped_column(USmallInt, ForeignKey("cities.city_id", ondelete="CASCADE", name="fk_alert_city"), nullable=False)
    target_role: Mapped[TargetRole] = mapped_column(Enum(TargetRole, values_callable=lambda e: [m.value for m in e]), nullable=False, default=TargetRole.OFFICER, server_default="officer")
    alert_type: Mapped[AlertType] = mapped_column(Enum(AlertType, values_callable=lambda e: [m.value for m in e]), nullable=False)
    severity: Mapped[AlertSeverity] = mapped_column(Enum(AlertSeverity, values_callable=lambda e: [m.value for m in e]), nullable=False, default=AlertSeverity.WARNING, server_default="warning")
    alert_message: Mapped[str] = mapped_column(Text, nullable=False)
    alert_status: Mapped[AlertStatus] = mapped_column(Enum(AlertStatus, values_callable=lambda e: [m.value for m in e]), nullable=False, default=AlertStatus.QUEUED, server_default="queued")
...
class AlertRecipient(Base):
    ...
    alert_id: Mapped[int] = mapped_column(UBigInt, ForeignKey("alerts.alert_id", ondelete="CASCADE", name="fk_ar_alert"), primary_key=True)
    user_id: Mapped[int] = mapped_column(UInt, ForeignKey("users.user_id", ondelete="CASCADE", name="fk_ar_user"), primary_key=True)
...
class PreventiveAction(Base):
    ...
    prediction_id: Mapped[Optional[int]] = mapped_column(UBigInt, ForeignKey("risk_predictions.prediction_id", ondelete="SET NULL", name="fk_action_prediction"))
    location_id: Mapped[int] = mapped_column(UInt, ForeignKey("locations.location_id", ondelete="CASCADE", name="fk_action_location"), nullable=False)
    user_id: Mapped[Optional[int]] = mapped_column(UInt, ForeignKey("users.user_id", ondelete="SET NULL", name="fk_action_user"))
    department: Mapped[Optional[str]] = mapped_column(String(120))
    action_taken: Mapped[Optional[str]] = mapped_column(Text)
    action_date: Mapped[date] = mapped_column(Date, nullable=False)
    status: Mapped[ActionStatus] = mapped_column(Enum(ActionStatus, values_callable=lambda e: [m.value for m in e]), nullable=False, default=ActionStatus.PLANNED, server_default="planned")
    created_at: Mapped[datetime] = mapped_column(TIMESTAMP, server_default=func.current_timestamp(), nullable=False)
