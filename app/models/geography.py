...
from sqlalchemy import (
    Boolean, CHAR, DateTime, Enum, ForeignKey, Index, Numeric, String,
...
class City(Base):
    ...
    country: Mapped[str] = mapped_column(CHAR(2), nullable=False, default="IN", server_default="IN")
    ...
    timezone: Mapped[str] = mapped_column(String(40), nullable=False, default="Asia/Kolkata", server_default="Asia/Kolkata")
    role: Mapped[CityRole] = mapped_column(Enum(CityRole, values_callable=lambda e: [m.value for m in e]), nullable=False)
    monsoon_start_month: Mapped[int] = mapped_column(UTinyInt, nullable=False, comment="local season start, for season_position feature")
...
class WeatherCell(Base):
    ...
    __table_args__ = (
        UniqueConstraint("model", "latitude", "longitude", name="uq_cell_coords"),
        Index("idx_cell_city", "city_id"),
    )

    cell_id: Mapped[int] = mapped_column(UInt, primary_key=True, autoincrement=True)
    city_id: Mapped[int] = mapped_column(USmallInt, ForeignKey("cities.city_id", ondelete="CASCADE", name="fk_cell_city"), nullable=False)
    model: Mapped[WeatherModel] = mapped_column(
        Enum(WeatherModel, values_callable=lambda e: [m.value for m in e]),
        nullable=False, default=WeatherModel.ERA5, server_default="era5",
...
class IngestionRun(Base):
    ...
    source_id: Mapped[int] = mapped_column(USmallInt, ForeignKey("data_sources.source_id", ondelete="RESTRICT", name="fk_run_source"), nullable=False)
    started_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)
    finished_at: Mapped[Optional[datetime]] = mapped_column(DateTime)
    rows_ingested: Mapped[int] = mapped_column(UInt, nullable=False, default=0, server_default=text("0"))
    rows_rejected: Mapped[int] = mapped_column(UInt, nullable=False, default=0, server_default=text("0"))
    status: Mapped[IngestionStatus] = mapped_column(Enum(IngestionStatus, values_callable=lambda e: [m.value for m in e]), nullable=False, default=IngestionStatus.RUNNING, server_default="running")
...
class FailureType(Base):
    ...
    __table_args__ = (
        UniqueConstraint("code", name="uq_failure_code"),
    )
    ...
    scope: Mapped[FailureScope] = mapped_column(Enum(FailureScope, values_callable=lambda e: [m.value for m in e]), nullable=False, default=FailureScope.SCHEMA_ONLY, server_default="schema_only")
...
class Location(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_location_cell", "cell_id"),
    )

    location_id: Mapped[int] = mapped_column(UInt, primary_key=True, autoincrement=True)
    city_id: Mapped[int] = mapped_column(USmallInt, ForeignKey("cities.city_id", ondelete="CASCADE", name="fk_location_city"), nullable=False)
    cell_id: Mapped[Optional[int]] = mapped_column(UInt, ForeignKey("weather_cells.cell_id", ondelete="SET NULL", name="fk_location_cell"), comment="nearest ERA5 weather cell")
    ...
    geom_level: Mapped[GeomLevel] = mapped_column(Enum(GeomLevel, values_callable=lambda e: [m.value for m in e]), nullable=False, default=GeomLevel.WARD, server_default="ward")

    # On the municipality's own published hotspot register?
    is_known_hotspot: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default=text("0"))
    hotspot_source: Mapped[Optional[str]] = mapped_column(String(120))
    first_listed_year: Mapped[Optional[int]] = mapped_column(USmallInt, comment="year it first appeared on the official list")
    last_listed_year: Mapped[Optional[int]] = mapped_column(USmallInt, comment="NULL if still listed, set when it drops off")

    elevation_m: Mapped[Optional[Decimal]] = mapped_column(Numeric(7, 2))
    imperviousness: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2), comment="percent, from land cover")
...
class InfrastructureAsset(Base):
    ...
    location_id: Mapped[int] = mapped_column(UInt, ForeignKey("locations.location_id", ondelete="CASCADE", name="fk_asset_location"), nullable=False)
    ...
    status: Mapped[AssetStatus] = mapped_column(Enum(AssetStatus, values_callable=lambda e: [m.value for m in e]), nullable=False, default=AssetStatus.UNKNOWN, server_default="unknown")
